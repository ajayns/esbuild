...
func parseFile(args parseArgs) {
	...
	pathToFilter := args.keyPath.String()
	loaderArgs := config.LoaderArgs{
		Path: args.keyPath,
	}

	// Apply loader plugins in order until one succeeds
	for _, plugin := range args.options.LoaderPlugins {
		if !args.keyPath.IsAbsolute && !plugin.MatchInternal {
			continue
		}

		if !plugin.Filter.MatchString(pathToFilter) {
			continue
		}
		...
		for _, msg := range result.Msgs {
			if plugin.Name != "" {
				msg.Text = fmt.Sprintf("[%s] %s", plugin.Name, msg.Text)
			}
			...
			if msg.Location != nil {
				clone := *msg.Location
				if clone.File == "" {
					clone.File = args.prettyPath
				} else {
					clone.File = args.res.PrettyPath(ast.NewPath(clone.File))
				}
				if clone.Line < 0 {
					clone.Line = 0
				}
				...
				msg.Location = logging.LocationOrNil(args.importSource, args.pathRange)
			}

			args.log.AddMsg(msg)
		}

		// Report errors thrown by the loader itself
		if result.LoaderError != nil {
			hasError = true
			text := result.LoaderError.Error()
			if plugin.Name != "" {
				text = fmt.Sprintf("[%s] %s", plugin.Name, text)
			}
			args.log.AddRangeError(args.importSource, args.pathRange, text)
		}
		...
		if result.OriginalPath != "" {
			source.PrettyPath = result.OriginalPath
		}
		break
	}
...
func defaultLoaderPlugin(options *config.Options, fs fs.FS, res resolver.Resolver) config.LoaderPlugin {
	return config.LoaderPlugin{
		...
		Callback: func(args config.LoaderArgs) config.LoaderResult {
			...
			// Special-case stdin
			if stdin != nil {
				return config.LoaderResult{
					Contents:     &stdin.Contents,
					OriginalPath: stdin.SourceFile,
					Loader:       stdin.Loader,
				}
			}

			// Read normal modules from disk
			if args.Path.IsAbsolute {
				loader := loaderFromFileExtension(options.ExtensionToLoader, fs.Base(args.Path.Text))
				if loader == config.LoaderNone {
					return config.LoaderResult{}
				}
				if contents, ok := res.Read(args.Path.Text); ok {
					return config.LoaderResult{
						Contents: &contents,
						Loader:   loader,
					}
				...
				return config.LoaderResult{
					LoaderError: fmt.Errorf("Could not read from file: %s", args.Path.Text),
				}
			}

			// Force disabled modules to be empty
			if strings.HasPrefix(args.Path.Text, "disabled:") {
				contents := ""
				return config.LoaderResult{
					Contents: &contents,
				}
...
func ScanBundle(log logging.Log, fs fs.FS, res resolver.Resolver, entryPaths []string, options config.Options) Bundle {
	...
	// The default loader plugin must come last
	options.LoaderPlugins = append(append(
		[]config.LoaderPlugin{},
		options.LoaderPlugins...),
		defaultLoaderPlugin(&options, fs, res))
	...
	for _, absPath := range entryPaths {
		prettyPath := res.PrettyPath(ast.Path{Text: absPath, IsAbsolute: true})
...
func (b *Bundle) generateMetadataJSON(results []OutputFile) []byte {
	...
	for _, result := range results {
		if len(result.jsonMetadataChunk) > 0 {
			...
			j.AddString(fmt.Sprintf("%s: ", printer.QuoteForJSON(
				b.res.PrettyPath(ast.Path{Text: result.AbsPath, IsAbsolute: true}))))
